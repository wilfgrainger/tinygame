...
test('Android landscape keeps touch controls inside the viewport', async ({ page }, testInfo) => {
  ...
  await page.goto('/');
  for (const selector of ['.move-pad','.action-button.jump']) { const box=await page.locator(selector).boundingBox(); expect(box).not.toBeNull(); expect(box!.x).toBeGreaterThanOrEqual(0); expect(box!.y).toBeGreaterThanOrEqual(0); expect(box!.x+box!.width).toBeLessThanOrEqual(915); expect(box!.y+box!.height).toBeLessThanOrEqual(412); }
});
