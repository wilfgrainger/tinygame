...
export class GameApp {
  ...
  start(bootstrap: WorldBootstrap) {
    ...
    const sun = new pc.Entity('Sun'); sun.addComponent('light', { type: 'directional', color: new pc.Color(1,0.95,0.82), intensity: 1.25, castShadows: true, shadowResolution: 1024 }); sun.setEulerAngles(45, 35, 0); app.root.addChild(sun);
    const camera = new pc.Entity('Camera'); camera.addComponent('camera', { clearColor: new pc.Color(0.63,0.79,0.9), farClip: 320, fov: 58 }); app.root.addChild(camera);

    const runtime = new WorldBuilder(app).build();
    const spawn = SPAWN_POINTS.find((p) => p.id === bootstrap.profile.lastSpawnId) ?? SPAWN_POINTS[0]!;
    ...
    const updateCamera = (position: Vec3, yaw: number, pitch: number) => {
      const distance = 9; const pitchRad = pitch * Math.PI / 180;
    ...
    app.on('update', (dt: number) => {
      ...
      if (bike.snapshot.mounted) {
        if (frame.interactPressed) { const dismount = bike.dismount(); if (dismount) player.resumeGrounded(dismount); }
        else { const b=bike.update(dt,frame); player.setExternal(b.position,b.yaw,'bike'); }
      } else if (raft.snapshot.mounted) {
        if (frame.interactPressed) { const dismount=raft.dismount(); if(dismount) player.resumeGrounded(dismount); }
        else { const r=raft.update(dt,frame); player.setExternal(r.position,r.yaw,'raft'); }
      } else {
      ...
      const b=bike.snapshot; runtime.bikeEntity.setPosition(b.position.x,b.position.y+0.75,b.position.z); runtime.bikeEntity.setEulerAngles(0,b.yaw*180/Math.PI,0);
      const r=raft.snapshot; runtime.raftEntity.setPosition(r.position.x,r.position.y,r.position.z); runtime.raftEntity.setEulerAngles(0,r.yaw*180/Math.PI,0);
      view.sync(snap, snap.mode==='bike'?1.0:snap.mode==='raft'?0.7:0);
      updateCamera(snap.position,snap.yaw,snap.pitch);

      for (const discovery of DISCOVERIES) if (!discovered.has(discovery.id) && Math.hypot(snap.position.x-discovery.position.x,snap.position.z-discovery.position.z)<=discovery.radius) {
        discovered.add(discovery.id); void this.api.saveDiscovery(discovery.id).then(()=>this.hud.showToast(discovery.id==='mountain-summit'?'You found the summit!':discovery.id==='woodland-grove'?'Secret grove discovered!':'Harbour lookout discovered!')).catch(()=>{ discovered.delete(discovery.id); this.hud.setSave('failed'); });
      }
    ...
    this.resizeHandler = () => { const dpr=Math.min(window.devicePixelRatio||1, matchMedia('(pointer: coarse)').matches?1.5:2); this.canvas.width=Math.floor(innerWidth*dpr); this.canvas.height=Math.floor(innerHeight*dpr); app.resizeCanvas(); };
    window.addEventListener('resize',this.resizeHandler); this.resizeHandler();
  }

  destroy() { this.keyboard?.destroy(); if(this.resizeHandler) window.removeEventListener('resize',this.resizeHandler); this.app?.destroy(); this.app=null; }
}
