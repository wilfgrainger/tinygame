...
export class BikeController {
  ...
  constructor(spawn: Vec3, private readonly collision: CollisionWorld) { this.spawn = { ...spawn }; this.state = { position: { ...spawn }, yaw: Math.PI, mounted: false, speed: 0 }; }
  ...
  update(dt: number, input: InputFrame) {
    ...
    const delta = { x: Math.sin(this.state.yaw) * this.state.speed * dt, y: 0, z: -Math.cos(this.state.yaw) * this.state.speed * dt };
    this.state.position = this.collision.resolveMove(this.state.position, delta, 0.9);
    return this.snapshot;
  }
  dismount() {
    if (!this.state.mounted) return null;
    const side = 1.5; const candidates = [
